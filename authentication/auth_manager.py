...
Dependencies:
- Requires: logging, data_models, decoders.jwt_decoder, _godcapture.core.har_compression, gzip
    ...
    def _extract_user_info(self, jwt_tokens: List[GongAuthenticationToken]) -> Optional[Dict[str, str]]:
        ...
        for token in jwt_tokens:
            if token.user_email and token.cell_id:
                # Extract workspace_id from cell_id (e.g., "us-14496" -> "14496")
                workspace_id = None
                if token.cell_id and '-' in token.cell_id:
                    workspace_id = token.cell_id.split('-')[-1]
                
                # Extract company_id from JWT payload 'gp' field if available
                company_id = None
                if hasattr(token, 'payload') and hasattr(token.payload, 'gp'):
                    company_id = token.payload.gp
