...
extracted data against ground truth data manually copied from Gong and organized
in the validation directories.

Ground truth data structure:
- Call data: app_backend/agent_tools/gong/validation/call_salesforce/
- Email data: app_backend/agent_tools/gong/validation/emails_salesforce/
...
- BLOCKING: This test MUST pass before considering Gong toolkit complete
"""
...
class GongRealDataValidator:
    ...
    def _parse_email(self, content: str) -> Dict[str, Any]:
        ...
        # Process sender info - extract email from name/title
        if sender_info:
            # First line is usually the name, look for email pattern or construct from name
            sender_name = sender_info[0]
            email_data['sender_name'] = sender_name

            # Look for company info to construct email
            company = None
            ...
            # Construct email from name and company
            if company and sender_name:
                # Convert "Brian Coons" to "brian.coons@postman.com"
        ...
        # Process recipient info - similar logic
        if recipient_info:
            # Find the main recipient (first name after 'to')
            recipient_name = None
            company = None

            for info in recipient_info:
                if any(title in info for title in ['Director', 'Analyst', 'Manager', 'Vice President', 'Rvp']):
                    # This is a title line, extract company
                    if 'Postman' in info:
                        company = 'postman.com'
                    elif 'Salesforce' in info:
                        company = 'salesforce.com'
        ...
        # Process CC info - extract names and construct emails
        if email_data['cc']:
            processed_cc = []
            current_name = None

            for cc_line in email_data['cc']:
                if any(title in cc_line for title in ['Analyst', 'Director', 'Vice President', 'Manager']):
                    # This is a title line, use with previous name
                    if current_name and 'Salesforce' in cc_line:
                        name_parts = current_name.lower().split()
                        if len(name_parts) >= 2:
                            processed_cc.append(f"{name_parts[0]}.{name_parts[1]}@salesforce.com")
                        else:
                            processed_cc.append(f"{name_parts[0]}@salesforce.com")
                    current_name = None
                else:
                    # This might be a name
                    if cc_line and not any(x in cc_line for x in ['EDT', 'EST', 'am', 'pm']):
                        current_name = cc_line

            email_data['cc'] = processed_cc
